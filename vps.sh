...
create_new_vm() {
    ...
    while true; do
        read -p "$(print_status "INPUT" "Disk size (default: 80G): ")" DISK_SIZE
    ...
    while true; do
        read -p "$(print_status "INPUT" "Memory in MB (default: 20480): ")" MEMORY
        MEMORY="${MEMORY:-20480}"
    ...
    while true; do
        read -p "$(print_status "INPUT" "Number of CPUs (default: 3): ")" CPUS
        CPUS="${CPUS:-3}"
